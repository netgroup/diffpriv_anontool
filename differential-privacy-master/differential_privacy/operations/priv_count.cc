...
int main(int argc, char **argv) {
  if (argc < 5)
    ...
    if (file.is_open()) {
      ...
      std::string true_val = absl::StrFormat("%f", op.BoundCount(0, 150));
      std::string priv_val = absl::StrFormat("%f", GetValue<int>(op.PrivateCount(budget, 0, 150).ValueOrDie().elements(0).value()));
