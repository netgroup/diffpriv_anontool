...
int main(int argc, char **argv) {
  if (argc < 5)
    ...
    if (file.is_open()) {
      // Create an object containing true and private counts
      std::string true_val = absl::StrFormat("%f", op.BoundCount(lower, upper));
      std::string priv_val = absl::StrFormat("%f", GetValue<int>(op.PrivateCount(budget, lower, upper).ValueOrDie().elements(0).value()));
      Result res(true_val, priv_val);
      // Write true and private counts on an output file
      file << res;
      file.close();
      return 0;
    }
