...
int main(int argc, char **argv) {
  if (argc < 5)
    ...
    if (file.is_open()) {
      ...
      std::string true_val = absl::StrFormat("%f", op.Sum(0, 150));
      std::string priv_val = absl::StrFormat("%f", GetValue<double>(op.PrivateSum(budget, 0, 150).ValueOrDie().elements(0).value()));
