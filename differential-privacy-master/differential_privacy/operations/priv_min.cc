...
int main(int argc, char **argv) {
  if (argc < 5)
    ...
    if (file.is_open()) {
      ...
      std::string true_val = absl::StrFormat("%f", op.Min(0, 150));
      std::string priv_val = absl::StrFormat("%f", GetValue<int>(op.PrivateMin(budget, 0, 150).ValueOrDie().elements(0).value()));
      Result res(true_val, priv_val);
      // Write true and private min values on an output file
      file << res;
      file.close();
      return 0;
    }
